...
int main() {
    ...
    char end_token = ')';
    char *enable_token = "do()";
    char *disable_token = "don't()";
    int execute = 1; // defaults to yes;
    int answer = 0;
    int count = 0;
    ...
    char *enable = strstr(ptr, enable_token);
    char *disable = strstr(ptr, disable_token);
    ...
    while ((ptr = strstr(ptr, key_token)) != NULL) {
        ...
        if (!execute) {
            if (!enable || enable > ptr) {
                continue;
            }
            disable = strstr(enable, disable_token);
        }

        if (disable && disable < ptr) {
            execute = 0;
            enable = strstr(disable, enable_token);
            continue;
        }
